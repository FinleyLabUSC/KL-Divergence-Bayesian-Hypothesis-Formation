...
def log_likelihood_baseline_and_timepoint(x):
    ...
    translocb = x[37:46]
    exp_data = np.load('posteriorapproximation/data_experimental.npy').flatten()
    exp_mu_pjak_12 = np.load('posteriorapproximation/exp_pjak_mu.npy')[-1]
    exp_mu_pstata_12 = np.load('posteriorapproximation/exp_pstata_mu.npy')[-1]
    exp_mu_pstatb_12 = np.load('posteriorapproximation/exp_pstatb_mu.npy')[-1]
    exp_mu_bcl = exp_data[26:33]
