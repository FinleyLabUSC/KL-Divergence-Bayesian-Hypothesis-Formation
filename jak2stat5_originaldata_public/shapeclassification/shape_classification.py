...
def shape_classification(socs_simulations):
    ...
    shape= ['early peak,\n transient:\n {}%'.format(int(percentage[0])), 'no peak,\n sustained:\n {}%'.format(int(percentage[1])), 'no peak,\n transient: {}%'.format(int(percentage[2])), 'early peak, sustained:\n {}%'.format(int(percentage[3]))]
    plt.rc('font', size=14)
    ...
    hypothesis = ['h10', 'h01', 'h00', 'h11']
    for i in np.arange(0, 4):
        np.save('shapeclassification/{}_simulations.npy'.format(hypothesis[i]), simulations[i])
