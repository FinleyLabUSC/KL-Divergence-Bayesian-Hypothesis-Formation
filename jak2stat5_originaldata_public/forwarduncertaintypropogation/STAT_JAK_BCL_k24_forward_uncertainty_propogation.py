...
def STAT_JAK_BCL_k24_forward_uncertainty_propogation(simulations, path, color, lower_percentile, upper_percentile, k24):
    ...
    pjak = simulations[4, :, :]
    bcl = simulations[5, :, :]
    exp_data = np.load('{}/data_experimental.npy'.format(path)).flatten()
    ...
    sim = [pstata, pstatb, transloca, translocb, pjak, bcl]
    exp = [exp_mu_pstata, exp_mu_pstatb, exp_mu_transloca, exp_mu_translocb, exp_mu_pjak, exp_mu_bcl]
    names = ['[pSTATA]', '[pSTATB]', 'N:C [STATA]', 'N:C [STATB]', '[pJAK]', '[BCL-xL]']
    exp_tspan = [exp_tspan_pjak_pstat, exp_tspan_pjak_pstat, exp_tspan_transloca, exp_tspan_translocb,
    ...
    for s, e, t, n, j in zip(sim, exp, exp_tspan, names, np.arange(1, 7)):
        ...
        plt.ylabel(' relative {}'.format(n), fontsize=10)
    plt.savefig('{}/JAK_STAT_BCL_timecourses_{}k24_{}th_to_{}th_percentile.png'.format(path, k24, lower_percentile*100, upper_percentile*100), dpi=300)