...
def split_r_convergence_metric(estimated_parameters1, estimated_parameters2, estimated_parameters3, estimated_parameters4, estimated_parameters5):
    gelman_rubin = []
    variable_dictionary = np.load('convergencediagnostics/dictionary_estimatedvariables.npy')
    for i in np.arange(0, 33):
        ...
        chain7 = np.log(estimated_parameters4[0:int(len(estimated_parameters1[:, i])/2), i])
        chain8 = np.log(estimated_parameters4[int(len(estimated_parameters1[:, i])/2):-1, i])
        chain9 = np.log(estimated_parameters5[0:int(len(estimated_parameters2[:, i])/2), i])
        chain10 = np.log(estimated_parameters5[int(len(estimated_parameters2[:, i])/2):-1, i])
